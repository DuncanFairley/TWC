...
mob/Player
	...
	verb
		Use_Statpoints()
			...
			if(usr.StatPoints>0)
				switch(input("Which stat would you like to improve?","You have [usr.StatPoints] stat points.")as null|anything in list ("Health", "Mana Points","Damage","Defense"))
					if("Health")
						if(src.StatPoints>0)
	...
	Stat()
		if(statpanel("Stats"))
			if(src.level>500)
				...
				stat("HP:","[src.HP]/[src.MHP+src.extraMHP] ([src.extraMHP/10])")
