//This file copyright Dan Bradley
...
DBConnection
	...
	proc
		...
		NewQuery(sql_query,cursor_handler=src.default_cursor)
			if(!my_connection.IsConnected())
				connected = my_connection.Connect(DBI,mysql_username,mysql_password)
				world.log << "[time2text(world.realtime)] - NewQuery() - MySQL server is disconnected - reconnecting [my_connection.ErrorMsg()]"
				if(!connected)
					world.log << "[time2text(world.realtime)] - NewQuery() - Tried to reconnect, and was unable to - Error: [my_connection.ErrorMsg()]"
