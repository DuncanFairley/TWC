...
swapmap
	...
	proc/Save()
		if(id==src) return 0
		var/savefile/S=mode?(new):new("[swapmaps_directory]/map_[id].sav")
		S << src
...
var/swapmaps_directory = "" //No preceeding /
...
proc/SwapMaps_Load(id)
	...
	if(!M)
		var/savefile/S
		var/text=0
		if(swapmaps_mode==SWAPMAPS_TEXT && fexists("[swapmaps_directory]/map_[id].txt"))
			text=1
		else if(fexists("[swapmaps_directory]/map_[id].sav"))
			S=new("[swapmaps_directory]/map_[id].sav")
		else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("[swapmaps_directory]/map_[id].txt"))
			text=1
		else return	// no file found
		if(text)
			S=new
			S.ImportText("/",file("[swapmaps_directory]/map_[id].txt"))
		S >> M
...
proc/SwapMaps_DeleteFile(id)
	fdel("[swapmaps_directory]/map_[id].sav")
	fdel("[swapmaps_directory]/map_[id].txt")

proc/SwapMaps_CreateFromTemplate(template_id)
	var/swapmap/M=new
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("[swapmaps_directory]/tmpl_[template_id].txt"))
		text=1
	else if(fexists("[swapmaps_directory]/tmpl_[template_id].sav"))
		S=new("[swapmaps_directory]/tmpl_[template_id].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("tmpl_[template_id].txt"))
		text=1
	else
		world.log << "SwapMaps error in SwapMaps_CreateFromTemplate(): [swapmaps_directory]/tmpl_[template_id] file not found."
		return
	if(text)
		S=new
		S.ImportText("/",file("[swapmaps_directory]/tmpl_[template_id].txt"))
	/*
...
proc/SwapMaps_LoadChunk(chunk_id,turf/locorner)
	...
	var/savefile/S
	var/text=0
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("[swapmaps_directory]/map_[chunk_id].txt"))
		text=1
	else if(fexists("[swapmaps_directory]/map_[chunk_id].sav"))
		S=new("[swapmaps_directory]/map_[chunk_id].sav")
	else if(swapmaps_mode!=SWAPMAPS_TEXT && fexists("[swapmaps_directory]/map_[chunk_id].txt"))
		text=1
	else
		world.log << "SwapMaps error in SwapMaps_LoadChunk(): [swapmaps_directory]/map_[chunk_id] file not found."
		return
	if(text)
		S=new
		S.ImportText("/",file("[swapmaps_directory]/map_[chunk_id].txt"))
	/*
		This hacky workaround is needed because S >> M will create a brand new
		M to fill with data. There's no way to control the Read() process
		properly otherwise. The //.0 path should always match the map, however.
...
proc/SwapMaps_GetSize(id)
	...
	if(swapmaps_mode==SWAPMAPS_TEXT && fexists("[swapmaps_directory]/map_[id].txt"))
		...
		world.log << "SwapMaps error in SwapMaps_GetSize(): [swapmaps_directory]/map_[id] file not found."
		return
	if(text)
		S=new
		S.ImportText("/",file("[swapmaps_directory]/map_[id].txt"))
